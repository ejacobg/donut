...
func (g *GIF) Print(w io.Writer) {
	_ = gif.Encode(w, g.Image, &g.Options)
}

// Gray16Palette returns 15 Gray16 colors plus a transparent color.
func Gray16Palette() color.Palette {
	palette := color.Palette{color.Alpha{}}
	for i := 0; i < 16; i++ {
		palette = append(palette, color.Gray16{Y: 1 << i})
	}
	return palette
}
...
func Gray16SF(L float64) (color.Color, bool) {
	...
	index := int(L*5) + 8
	return color.Gray16{Y: 1 << index}, true
}

// MonoSF returns a SetFunc that always applies a single color.
